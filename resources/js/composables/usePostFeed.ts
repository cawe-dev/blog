...
export const usePostFeed = (postsProp: Ref<IPostGroup>) => {
    ...
    const filteredPosts = computed(() => {
        ...
        if (searchQuery.value) {
            ...
            posts = posts.filter((p) => p.title.toLowerCase().includes(query) || p.excerpt?.toLowerCase().includes(query));
        }

        if (selectedCategories.value.length > 0) {
            posts = posts.filter((p) => p.category.slug && p.category.slug?.includes(selectedCategories.value));
        }

        if (selectedTags.value.length > 0) {
            posts = posts.filter((p) => p.tags && p.tags.some((tag) => selectedTags.value.includes(tag.slug)));
        }
